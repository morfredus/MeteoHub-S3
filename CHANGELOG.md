# [1.0.180] – 2026-03-01
1. Added optional ESP32 Dev Module HSPI SD mapping (`ESP32_DEV_SD_HSPI`) and a dedicated `esp32-dev-oled-hspi-debug` environment for bus fallback diagnostics.
2. Reworked SD wakeup clock generation to GPIO clock toggling (no raw `0xFF` SPI transfers), preventing serial monitor binary pollution warnings.
3. Kept SD manager on selected dedicated SPI bus and improved bus-name diagnostics for VSPI/HSPI fallback analysis.
