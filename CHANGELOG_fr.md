...
Version minimale valide : 1.0.158

## Version 1.0.158
1. **Correctif (Watchdog UI Web)** : Renforcement du traitement `/api/history` avec bornage des paramètres, limite du nombre de points de réponse et yields coopératifs dans les boucles longues pour éviter les resets watchdog `async_tcp`.
2. **Correctif (Watchdog API fichiers)** : Ajout de yields coopératifs et d'une limite de listing dans `/api/files/list` pour éviter les boucles bloquantes sur gros répertoires.
3. **Stabilité (Web asynchrone)** : Conservation du comportement fonctionnel des APIs existantes tout en réduisant la monopolisation CPU des handlers asynchrones.
