# [1.0.180] – 2026-03-01
1. Ajout d’un mapping SD HSPI optionnel pour ESP32 Dev Module (`ESP32_DEV_SD_HSPI`) et d’un environnement dédié `esp32-dev-oled-hspi-debug` pour le diagnostic de fallback bus.
2. Refonte de la génération des clocks de réveil SD via bascule GPIO (sans transferts SPI bruts `0xFF`), évitant les avertissements de pollution binaire dans le moniteur série.
3. Maintien du SD manager sur le bus SPI dédié sélectionné avec diagnostics enrichis du nom de bus pour l’analyse VSPI/HSPI.
