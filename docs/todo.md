...
- [ ] Replace blocking UI waits (`delay(1000/2000)`) in OLED flows with a non-blocking state machine based on `millis()`.
- [ ] Introduce a shared cooperative yield helper/macro (`delay(0)` every N iterations) and use it consistently in long history/files loops.
