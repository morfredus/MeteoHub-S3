# Index de la documentation

Version minimale valide : 1.0.139


## Environnements OLED/LCD
Toute la documentation couvre à la fois les environnements OLED (SH1106) et LCD (ST7789). Les différences de navigation, de câblage et d’interface sont expliquées dans chaque section.

## Commencer ici
- [Prise en main](getting_started_fr.md)
- [Câblage matériel](hardware_wiring_fr.md)
- [Configuration](configuration_fr.md)

## Utilisation quotidienne
- [Guide utilisateur](user_guide_fr.md)
- [Maintenance et dépannage](maintenance_and_troubleshooting_fr.md)
- [FAQ](faq_fr.md)

## Compréhension technique
- [Architecture du projet](project_architecture_fr.md)

## Documentation anglaise
Chaque document ci-dessus possède un équivalent anglais strict sans suffixe `_fr`.

- [Index (EN)](index.md)
- [Getting Started (EN)](getting_started.md)
- [Hardware Wiring (EN)](hardware_wiring.md)
- [Configuration (EN)](configuration.md)
- [User Guide (EN)](user_guide.md)
- [Project Architecture (EN)](project_architecture.md)
- [Maintenance and Troubleshooting (EN)](maintenance_and_troubleshooting.md)
- [FAQ (EN)](faq.md)
