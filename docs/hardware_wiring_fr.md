...
Le gestionnaire SD supporte un mode de bus configurable via `SD_BUS_MODE` dans `board_config.h` :
- `0` : AUTO (SPI puis SDIO)
- `1` : SPI uniquement
- `2` : SDIO uniquement

Le réglage actuel de cette carte est `SPI uniquement` pour éviter les tentatives SDIO inutiles sur les modules câblés en SPI.
